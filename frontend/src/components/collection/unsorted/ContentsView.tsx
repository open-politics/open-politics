...
export function ContentsView({
  ...
  const [dateRange, setDateRange] = useState<DateRange | undefined>({
    from: addDays(new Date(), -365), // Default to last month
    to: new Date()
  });
  ...
  // Add this useEffect to synchronize selectedEventType when highlightedEventType changes
  useEffect(() => {
  ...
  const filteredAndSortedContents = contents
    .filter(content => {
      ...
      if (!contentDate) return matchesSearch && matchesEventType; // Include undated content when filtering
  ...
  // Get unique event types from contents
    ...
    <div className="space-y-4 flex flex-col h-full">
      ...
      {selectedEventType && selectedEventType !== 'all' && (
        <div className="text-sm text-blue-500">
          Highlighting: {selectedEventType}
        </div>
      )}
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="text-sm text-muted-foreground">
          ...
          Visible contents: {filteredAndSortedContents.length}
        </div>
      )}
